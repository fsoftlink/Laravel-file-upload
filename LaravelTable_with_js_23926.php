trait ActionTrait
{
    protected array $allowedModels = [ 'CustomerAddress', 'Product' ];

    public function commonAction(Request $req) : array {
        try{
            if(2 == 3){    return ['status' => 'error', 'message' => "Stopped : $req->tbl", 'data'=> $req->all()];   }
            
            $task = $req->task ?? 'na';
            if(!in_array($task, ['edit','delete','fetch'])){
                return ['status' => 'error', 'message' => "Invalid task name",  'data'=> $req->all()];
            }  

            $rules = [
                'pk' => 'required|string',
                'pkv' => 'required|string',  
                'tbl' => 'required|string', // model name
            ];
            if(in_array($task, ['edit'])){
                $rules['name'] = 'required|string|min:3|max:40';
                $rules['value'] = 'required|string';
            }

            $validator = Validator::make($req->all(),$rules);
            if($validator->fails()){
                return ['status' => 'error', 'message' => $validator->errors()->first(), 'errors' => $validator->errors()];
            }

            $data = $req->all();
            if (!in_array($req->tbl, $this->allowedModels)) {
                return['status' => 'error','message' => 'Invalid or unauthorized table specified.','data'=> $data];
            }

            $modelClass = "App\\Models\\" . $req->tbl;
            //$rows = $modelClass::where('user_id', $user->email)->get();
            //$model = app($modelClass);     //$rows = $model->where('user_id', $user->email)->get();

            if (!class_exists($modelClass)) {
                return ['status' => 'error', 'message' => "Model Select Class not found!", 'data'=> $data];
            }
            $user = Auth::user();
            if (!$user) {   return ['status' => 'error', 'message' => "Please Login", 'data'=> $data];    }
            ...
            $row = $modelClass::where($pk, $pkv)->first();
            if(empty($row)){
                return ['status' => 'error', 'message' => "No Record Found in Model : $req->tbl", 'data'=> $data];
            }if($row->user_id != $user->email){
                return ['status' => 'error', 'message' => "Invalid record trying to fetch", 'data'=> $data];
            }

            if($task == 'edit'){
                $row->{$name} = $value;
                $row->save();
                return ['status' => 'success', 'message' => "Record Update Success", 'data'=> $data, 'reload'=>'reload'];
            }
            else if($task == 'fetch'){
                return ['status' => 'redirect', 'message' => "Record Fetch Success", 'url' => route('front.userShippingAddress', ['id' => $row->id]) ];
                //return ['status' => 'redirect', 'message' => "Record Fetch Success", 'url'=> route("front.userShippingAddress/{$pkv}") "/ship-address/{$pkv}"];
            }
            else if($task == 'delete'){
                $row->delete();
                return ['status' => 'success', 'message' => "Record Delete Success", 'row'=> $row,'reload'=>'reload'];
            }  
            return ['status' => 'error', 'message' => "No task done", 'row'=> $row];
        } catch (Exception $e) {
            return ['status' => 'error', 'message' => "Error occured. Please check in trait", 'data'=> $req->all()];
        }//finally{       }    
    }
}
...
{ 
    use ActionTrait;
    public function rowAction(Request $req){
        $ans = $this->commonAction($req);
        //if($ans['status'] == 'error'){     return edie($ans);     }
        return edie($ans); 
    }
}
...
{
	public function userShippingAddress($id = 0)
    {
        $user = Auth::user();
        ...
        if($id != 0){    
            $address = CustomerAddress::where('id', $id)
                    ->where('user_id', $user->email) // Ya $user->id
                    ->first();   
            if(empty($address)){
                 $address =  new CustomerAddress();
            }
        ...
        foreach ($rows as $key => $row) {
            ...
            $del = json_encode(['pk'=> 'id' ,'pkv'=>$row->id,'tbl'=> 'CustomerAddress', 'task'=> 'delete', 'action'=> route('front.rowAction')]);
            $edit = json_encode([
                'pk'=> 'id' ,'pkv'=>$row->id, 'task'=> 'fetch',
                'tbl'=> 'CustomerAddress', 'action'=> route('front.rowAction'),
            ]);
            $input = json_encode([
                'pk'=> 'id' ,'pkv'=>$row->id, 'task'=> 'edit', 'name'=> 'first_name' ,'value'=> $row->first_name, 
                'tbl'=> 'CustomerAddress', 'action'=> route('front.rowAction'),
            ]);
            $select = json_encode([
                'pk'=> 'id' ,'pkv'=>$row->id, 'task'=> 'edit', 'name'=> 'status' ,'value'=> $row->first_name, 
                'tbl'=> 'CustomerAddress', 'action'=> route('front.rowAction'),
            ]);
            $option = [
                ['label'=>'Active', 'value'=>'Active', 'selected'=>$row->status == 'Active'],
                ['label'=>'Dective', 'value'=>'Dective',  'selected'=>$row->status == 'Dective'],
            ];
            $temp = [
                'sl' => $key+1,
                'pk' => $row->id,
                ...
                '_edit' => ['payload'=>$edit, 'func'=>'myPost'],
                '_input' => ['payload'=>$input, 'func'=>'myPostAsk','type'=> 'text', 'value'=>$row->first_name ], 
                '_select' => ['payload'=>$select,  'func'=>'myPostAsk','type'=> 'select', 'option'=>$option ], 
            ];
...
function smsTable($resData = [], $divId = '') {
    ...
    $checkbox = $resData['checkbox'] ?? false;
    ...
    echo "<table class='smsTbl table table-striped table-sm table-bordered align-middle'>";
    echo "<thead><tr>";
    ...
    foreach ($tbody as $keyName => $row) {
        ...
        foreach ($keys as $key) {
            ...
            if (str_ends_with($key, '_del')) {
                $rowVal = "<button class='btn btn-sm btn-danger shiny' data-payload='{$payload}' data-func='{$funcName}'>Delete</button>";
            }
            if (str_ends_with($key, '_edit')) {
                $rowVal = "<button class='btn btn-sm btn-primary shiny' data-payload='{$payload}' data-func='{$funcName}'>Edit</button>";
            }
            if (str_ends_with($key, '_input')) {
                $rowVal = "<input type='{$inputType}' class='form-control' value='{$inputVal}' data-payload='{$payload}' data-func='{$funcName}' style='width:100px;' >";
            }
            if (str_ends_with($key, '_select')) {
                ...
                if (is_array($selectOption)) {
                    foreach ($selectOption as $option) {
                        ...
                        $optionHtml .= "<option {$selected} value='{$option['value']}'>{$option['label']}</option>";
                    }
                }

                $rowVal = "<select class='form-select form-select-sm' data-payload='{$payload}' data-func='{$funcName}' style='width:100px;'>
                                {$optionHtml}
                            </select>";
            }
